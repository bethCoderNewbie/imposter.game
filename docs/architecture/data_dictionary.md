...
**Last updated:** 2026-04-15 (schema v0.9: Wolf charge attack — 2 new `NightActions` server-only fields (`wolf_charges`, `charge_kill_target_id`), 1 new `PlayerState` field (`under_attack`), 1 new `EliminationCause` (`grid_charge_kill`), 2 new error codes — PRD-015, ADR-026. Previously: schema v0.8 Night Grid system — PRD-013, ADR-025)
**Schema version:** 0.9
...
**`match_data` payload** (server → all display sockets, lobby phase only):
...
| `INVALID_GRID_COORDS` | `select_grid_node` intent supplied `row` or `col` outside [0, 4], or not integers |
| `NODE_OCCUPIED` | `select_grid_node` targeted a node already in `grid_activity` (already solved this night) |
| `PUZZLE_ALREADY_ACTIVE` | `select_grid_node` received while `grid_puzzle_state.active == true` — finish the current node first |
| `NOT_YOUR_TURN` | `select_grid_node` received from a player with `wakeOrder != 0`; grid is `wakeOrder == 0` only |
| `INVALID_QUADRANT` | `sonar_ping` quadrant not in `{"top_left","top_right","bottom_left","bottom_right"}` |
| `NOT_WOLF` | `sonar_ping` received from a non-wolf-team player |
| `SONAR_PING_LIMIT_REACHED` | Pack has fired all 4 Sonar Pings this night (`sonar_pings_used >= 4`) |
| `GRID_NOT_READY` | `select_grid_node` received before `grid_layout` was generated; indicates a phase-sync issue |
| `INVALID_CHARGE` | `wolf_charge_update` — `accumulated_ms` is not a non-negative number |
| `INVALID_ACTION` (grid_defend) | `grid_defend` received from a wolf-team player |
...
| `seer_knowledge` | `map[str → str]` | Maps target `player_id` → `investigationResult` string for every player the Seer has investigated. Accumulates across rounds; never cleared until game over. Values: `"village"` \| `"werewolf"` \| `"neutral"`. Note: reflects `investigationResult` — Alpha Wolf returns `"village"` here. **Present in the Seer's stripped state during both NIGHT and DAY phases.** Used by `DayDiscussionScreen` (`SeerIntelPanel`) to display accumulated intel during discussion. See ADR-015, PRD-007. | Mutated by `resolve_night()` when processing seer's `seer_target_id` | No (empty map until Seer acts) | **Yes — sent only to the Seer; removed for all others (both NIGHT and DAY phases)** |
...
| `difficulty_level` | `DifficultyLevel` (`"easy"\|"standard"\|"hard"`) | Role composition bias preset. Maps to `targetBalanceRange` passed to `build_composition()`. `easy=[0,4]` (village-favorable), `standard=[-2,2]` (balanced), `hard=[-4,0]` (wolf-favorable). Default: `standard`. | Set by host in lobby via `PATCH /api/games/{id}/config`; frozen at game start. The derived `targetRange` is ephemeral — not stored in state. | No |
| `narrator_voice` | `str` | Subdirectory under `api/narrator/audio/` to use for prebaked WAVs. Default: `"kokoro"`. | Set by host via `PATCH /api/games/{id}/config`; validated against on-disk subdirs at write time. Frozen at game start. | No |

---

## PlayerRosterEntry

Minimal player descriptor sent in `match_data` payloads (ADR-009). Intentionally omits all secret fields (`role`, `team`, `night_action_submitted`, `vote_target_id`, and all server-only fields).

| Field | Type | Description | Source / Logic | Nullable | HiddenFromPlayer |
|:------|:-----|:------------|:---------------|:---------|:----------------|
| `player_id` | `str` (UUID) | Unique player identifier — matches `PlayerState.player_id`. | `POST /games/{id}/join` | No | No |
| `display_name` | `str` | Player-chosen name. Max 16 characters. | Player-supplied at join | No | No |
| `avatar_id` | `str` | Asset key into the static avatar registry. E.g., `"wolf_01"`. | Player-chosen at onboarding | No | No |
| `is_connected` | `bool` | Live connection status at the time `broadcast_roster()` fires. | `ConnectionManager` at broadcast | No | No |

> **Display client Zustand store:** The Display client stores the current roster in `useGameStore.roster` (a Zustand store seeded from `sync.state.players` on first connect, then updated on every `match_data` message). `LobbyScreen` reads exclusively from this store — it does not diff `gameState.players`.
...
| `grid_node_row` | `int` \| `null` | Row index (0–4) of the Data Node the player is currently solving on the night grid. `null` when the player is not actively solving a grid puzzle. Reset to `null` on wrong answer, correct answer, and every night phase entry. | Set by `handle_select_grid_node` intent; cleared on answer or phase reset | Yes | **Yes — server-only; never sent to any client** |
| `grid_node_col` | `int` \| `null` | Column index (0–4) of the active grid node. Companion to `grid_node_row`. Same lifecycle. | Set by `handle_select_grid_node` intent; cleared on answer or phase reset | Yes | **Yes — server-only; never sent to any client** |
| `grid_puzzle_state` | `PuzzleState` \| `null` | Active grid node puzzle for this player. Tier-specific (`time_limit_seconds`: 5 / 10 / 20 for Tier 1 / 2 / 3). Distinct from `puzzle_state` (Archive). `null` for wolves, dead players, and outside the night phase. Seeded per-node: `f"{G.seed}:{G.round}:{player_id}:{row}:{col}:grid_puzzle"`. | Set by `handle_select_grid_node` when a node is tapped; cleared on answer or phase reset | Yes | **Yes — sent only to own player; `correct_index` stripped before broadcast; `null` for wolves and all other players** |
| `under_attack` | `bool` | `true` while a wolf is actively holding charge on the quadrant this player is currently solving in. Triggers the `AttackWarningOverlay` on the player's mobile device. `false` when the wolf releases, the charge fires (quadrant disrupted), the villager successfully defends, or the player is not solving a node. Reset to `false` on every night phase entry. | Set/cleared by `handle_wolf_charge_update` (via `_players_in_quadrant`); cleared by `handle_grid_defend`; reset by `transition_phase("night")` | No | **Yes — sent only to own player (wakeOrder == 0). Always `false` in wolf views and Display client.** |
...
| `seer_result` | `str` enum \| `null` | Result of the Seer's inspect this round. Returns `investigationResult` field of the target — `"village"` \| `"wolf"` \| `"neutral"`. **Uses `investigationResult`, not `team`** — Alpha Wolf returns `"village"` here. Stored transiently for broadcast; cumulative record lives in `MasterGameState.seer_knowledge`. **Present in both the intermediate NIGHT broadcast and the subsequent DAY broadcast** — `night_actions` is not reset at the DAY transition; it is only reset on the next NIGHT entry. The Seer's stripped DAY state therefore always contains this field during day discussion. See ADR-015. | Computed by `resolve_night()` from `players[seer_target_id].investigationResult` | Yes | **Yes — sent only to the Seer (both NIGHT and DAY phases until next NIGHT entry)** |
...
| `grid_layout` | `array[array[int]]` \| `null` | 5×5 tier grid generated at night phase entry. Each cell is `1` (green, 5 s), `2` (yellow, 10 s), or `3` (red, 20 s). Distribution is fixed: exactly 18 green, 6 yellow, 1 red per round. Seeded deterministically: `f"{G.seed}:{G.round}:grid_layout"`. `null` outside the night phase. | Generated by `generate_grid_layout(seed, round)` in `engine/puzzle_bank.py`; set by `transition_phase("night")` | Yes | No — public; sent to all views. The tier map itself contains no secret information. |
| `grid_activity` | `array[dict]` | Append-only log of completed Data Nodes this night. Each entry: `{"row": int, "col": int, "quadrant": str, "sequence_idx": int}`. **No `player_id` is ever written here** — this is the primary security invariant ensuring wolves cannot identify which specific villager solved a node. `sequence_idx` is the entry's 0-based position in the list. | Appended by `handle_submit_grid_answer` on a correct solve; cleared by `transition_phase("night")` | No (empty array) | **Yes — wolf-team players only. Removed for villagers, neutrals, and Display client.** |
| `sonar_pings_used` | `int` | Total Sonar Pings the whole wolf pack has fired this night. The pack shares a hard cap of `MAX_SONAR_PINGS_PER_NIGHT = 4` (one per quadrant). `handle_sonar_ping` raises `SONAR_PING_LIMIT_REACHED` once this reaches 4. Reset to `0` at each night phase entry. Public — all clients see this, enabling the `night_recap` Tier 1 hint to report it to villagers. | Incremented by `handle_sonar_ping`; server rejects at 4 | No | No — public |
| `sonar_ping_results` | `array[dict]` | List of sonar ping result objects accumulated by the wolf team this night. Each entry: `{"quadrant": str, "heat": int, "tier_counts": {"1": int, "2": int, "3": int}}`. `heat` = number of completed nodes in that quadrant at ping time. `tier_counts` breaks down activity by node tier. | Appended by `handle_sonar_ping`; cleared by `transition_phase("night")` | No (empty array) | **Yes — wolf-team players only. Never sent to villagers or Display client.** |
| `night_action_change_count` | `map[str → int]` | Maps `player_id` → number of intent submissions this player made during the night phase. Counts every `select_grid_node` and `sonar_ping` submission. Used exclusively by the Tier 3 `action_log` hint category — the hint reveals the maximum count without naming the player. Reset to `{}` at each night phase entry. | Incremented by `handle_select_grid_node` and `handle_sonar_ping` | No (empty map) | **Yes — server-only; never sent to any client under any view** |
| `wolf_charges` | `map[str → map[str → int]]` | Pack charge accumulator: `wolf_pid → {quadrant → accumulated_ms}`. Records each wolf's cumulative hold time per quadrant this night. When the pack total for any quadrant crosses `CHARGE_THRESHOLD_MS` (5000 ms), `_apply_charge_fire()` fires: `charge_kill_target_id` is set, active solvers in that quadrant are disrupted, and all wolves' charges for that quadrant reset to `0`. Defend also resets the pack's charges for the defender's quadrant. Reset to `{}` at every night phase entry. | Written by `handle_wolf_charge_update`; pack total checked and reset on fire/defend | No (empty map) | **Yes — server-only; never sent to any client** |
| `charge_kill_target_id` | `str` \| `null` | PID of the first active solver in a quadrant where the pack charge auto-fired. Set when `_pack_charge_total(quadrant) >= 5000`. Consumed by `_step7_wolf_kill_or_infect()` in `resolve_night()` — charge kill takes priority over wolf vote kill. Cleared to `null` at night phase entry and after resolution. | Set by `_apply_charge_fire()` in `handle_wolf_charge_update`; consumed and cleared by `_step7_wolf_kill_or_infect` | Yes | **Yes — server-only; always stripped; never sent to any client** |

> **Grid security invariant:** `grid_activity` entries intentionally omit `player_id`. Attacker modification of client code cannot recover which player solved which node because the information was never serialized into the broadcast payload. The stripping rule for wolves provides location data only (quadrant + tier), which is sufficient for radar gameplay without enabling targeted identification.
>
> **Grid layout vs. Archive puzzle coexistence:** `grid_layout` and `grid_activity` are additive — they coexist with the existing `puzzle_state` (Archive) system per ADR-025. Solving an Archive puzzle does not mark a grid node as completed, and solving a grid node does not affect the Archive puzzle. Both can be active for the same player simultaneously.
>
> **Framer and grid hints:** The Framer's `hack_archives` action targets Archive puzzle delivery only (`puzzle_state`). Grid node hint delivery (`grid_puzzle_state`) is not intercepted by the Framer in the current implementation. See PRD-013 §7 open question #3.
...
| `cause` | `str` enum | How they died. `"wolf_kill"` \| `"village_vote"` \| `"arsonist_ignite"` \| `"serial_killer_kill"` \| `"broken_heart"` \| `"hunter_revenge"` \| `"witch_kill"` \| `"lunatic_sacrifice"` \| `"lunatic_curse"` \| `"bodyguard_kill"` \| `"bodyguard_sacrifice"` \| `"grid_charge_kill"`. `grid_charge_kill`: wolf pack held charge on the victim's quadrant for a combined 5000 ms; subject to the same Doctor / Wise shield / Bodyguard protection checks as a standard wolf kill. | Set by resolver | No | No |
...
| `puzzle_data` | `object` | Type-specific puzzle content. **math:** `{ "expression": str, "answer_options": [str, str, str], "correct_index": int }` — 3 options. **logic:** `{ "question": str, "answer_options": [str, str, str, str], "correct_index": int }` — 4 options; question drawn from `puzzles.md` static bank (400 trivia Q&A in 11 categories); 3 distractors are drawn from the same category as the question for domain-coherent plausibility (ADR-012). **sequence:** `{ "sequence": [str] }` — list of color IDs to replay. | No |
